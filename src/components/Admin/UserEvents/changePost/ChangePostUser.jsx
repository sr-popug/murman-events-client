...
export default function ChangePostUser() {
	...
	useEffect(() => {
		axios
			.get(`http://localhost:5001/api/offerposts/${params}`)
			.then((res, req) => {
				setPost(res.data)
			})
	...
	const handleSubmit = e => {
		...
		try {
			...
			axios.post(`http://localhost:5001/api/posts`, formData).then(() => {
				navigate('/articles')
				axios.delete(`http://localhost:5001/api/offerposts/delete/${params}`)
			})
		} catch (e) {
			setError('Какие-то из полей не заполнены')
		}
